...
public class ChatBot extends TelegramLongPollingBot {
    ...
    public String getBotUsername() {
        return "MyBanderoGooseBot";
    }

    @Override
    public String getBotToken() {
        return "5806651059:AAG_76yDnthix1CxdUJImVCg8lZP5lfW9M8";
    }
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasMessage() && update.getMessage().getText().equals("/start")) {
            ...
            SendMessage message = createMessage("Ґа-ґа-ґа!\n" +
                    ...
                    "\n" +
                    "Ти отримуєш гусака №" + (int)((Math.random()* 125) + 1) + "\n" +
                    "\n" +
            ...
            attachButtons(message, Map.of(
                    buttons.get(0), "level_1",
                    buttons.get(1), "level_1",
                    buttons.get(2), "level_1"
        ...
        if (update.hasCallbackQuery()) {
            if (update.getCallbackQuery().getData().equals("level_1") && getLevel(chatId) == 1) {
                ...
                attachButtons(message, Map.of(
                        buttons.get(0), "level_2",
                        buttons.get(1), "level_2",
                        buttons.get(2), "level_2"
                ));
                sendApiMethodAsync(message);
            }
            if (update.getCallbackQuery().getData().equals("level_2") && getLevel(chatId) == 2) {
                ...
                attachButtons(message, Map.of(
                        buttons.get(0), "level_3",
                        buttons.get(1), "level_3",
                        buttons.get(2), "level_3"
                ));
                sendApiMethodAsync(message);
            }
            if (update.getCallbackQuery().getData().equals("level_3") && getLevel(chatId) == 3) {
                ...
                message.setChatId(chatId);
                attachButtons(message, Map.of(
                        "Купити Джавелін (50 монет)", "level_4"
                ));

                sendApiMethodAsync(message);
            }
            if (update.getCallbackQuery().getData().equals("level_4") && getLevel(chatId) == 4) {
                setLevel(chatId, 5);
